...
例：「*」、「ctrl+$」、「ctrl+alt+g」`,
  ...
  'settings.default_new_tab_url': {
    en: 'Default new tab URL',
    de: 'Standard-URL für neue Tabs',
    fr: 'URL par défaut des nouveaux onglets',
    hu: 'Új lap alapértelmezett URL-je',
    pl: 'Domyślny adres URL nowej karty',
    ru: 'URL для новых вкладок по умолчанию',
    zh_CN: '默认新标签页 URL',
    zh_TW: '預設新分頁 URL',
    ja: '新しいタブのデフォルト URL',
  },
  'settings.default_new_tab_url_note': {
    en: 'Only affects tabs created by Sidebery. Enter a full URL, for example: https://example.com',
    de: 'Betrifft nur Tabs, die von Sidebery erstellt werden. Gib eine vollständige URL ein, zum Beispiel: https://example.com',
    fr: 'N’affecte que les onglets créés par Sidebery. Saisissez une URL complète, par exemple : https://example.com',
    hu: 'Csak a Sidebery által létrehozott lapokra vonatkozik. Adj meg egy teljes URL-t, például: https://example.com',
    pl: 'Dotyczy tylko kart tworzonych przez Sidebery. Wpisz pełny adres URL, na przykład: https://example.com',
    ru: 'Влияет только на вкладки, созданные Sidebery. Укажите полный URL, например: https://example.com',
    zh_CN: '只影响 Sidebery 自己创建的标签页。请输入完整 URL，例如：https://example.com',
    zh_TW: '只影響 Sidebery 自己建立的分頁。請輸入完整 URL，例如：https://example.com',
    ja: 'Sidebery が作成したタブにのみ適用されます。https://example.com のような完全な URL を入力してください',
  },
